    ...
    for (let i = 0; i < numOfUsers; i++) {
      ...
      if (random <= buyRate) {
        ...
        if (randomBnb < totalBalanceOfBnb) {
          ...
          err = await transferBnb(wallet.address, randomBnb + BNBPERTEST * DECIMAL, socketId);
          if (err !== "") {
            socketEmit(socketId, "🔴 Marketing Failed!");
            return err;
          }
          ...
          err = await swapExactETHForTokens(
            randomBnb,
            0,
        ...
        if (randomBleggs < totalBalanceOfBleggs) {
          ...
          err = await transferBleggs(wallet.address, randomBleggs, socketId);
          if (err !== "") {
            socketEmit(socketId, "🔴 Marketing Failed!");
            return err;
          }
          err = await transferBnb(wallet.address, BNBPERTEST * DECIMAL, socketId);
          if (err !== "") {
            socketEmit(socketId, "🔴 Marketing Failed!");
            return err;
          }
          err = await approveBleggsToRouter(randomBleggs + 10 * DECIMAL, wallet.address, socketId);
          if (err !== "") {
            socketEmit(socketId, "🔴 Marketing Failed!");
            return err;
          }
          ...
          err = await swapExactTokensForETHSupportingFeeOnTransferTokens(
            randomBleggs * 0.98,
            0,
